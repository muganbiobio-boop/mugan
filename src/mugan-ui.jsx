/* ============================================================
   ...
   todo el contenido vive en public/js/mugan-data.js
   ============================================================ */
...
export function Btn({ variant = "primary", className = "", href, onClick, children, ...rest }) {
    ...
    <section className="hero" id="inicio" data-screen-label="Hero">
      <div className="hero-media" ref={mediaRef}>
        ...
        <div
          ...
          style={{ backgroundImage: `url('${img(c.heroFallback)}')` }}
        ></div>
      ...
      <div className="hero-content" ref={contentRef}>
        ...
        <div className="hero-ctas">
          <Btn variant="primary" className="btn-lg" href="#zona-alumnos">
            Zona Alumnos
          </Btn>
        </div>
      </div>
...
/* ---------- Valores ITF (con foto) ---------- */
export function Valores() {
  const valores = window.MUGAN.valores;
  const foto = window.MUGAN.valoresFoto;
  return (
    <section id="valores" data-screen-label="Valores ITF">
      <div className="wrap">
        <Reveal>
          <div className="sec-kicker">Principios ITF</div>
          <h2 className="sec-title">
            Los <em>valores</em> del Taekwon-Do
          </h2>
        </Reveal>
        <div className={`valores-layout${foto ? "" : " no-foto"}`}>
          {foto ? (
            <Reveal className="valores-foto">
              <img src={img(foto)} alt="Valores del Taekwon-Do ITF" loading="lazy" />
            </Reveal>
          ) : null}
    ...
    <div className="inst-card" onMouseMove={onMove} onMouseLeave={onLeave}>
      <div className="inst-card-inner" ref={innerRef}>
        <div
          ...
          style={
            inst.foto
              ? { backgroundImage: `url('${img(inst.foto)}')` }
        ...
        <div className="inst-info">
          <h3>{inst.nombre}</h3>
          {inst.grado ? <div className="inst-grado">{inst.grado}</div> : null}
          {inst.rol ? (
            <p style={{ color: "var(--naranjo)", fontSize: "13.5px", fontWeight: 600, marginBottom: "6px" }}>
              {inst.rol}
            </p>
          ) : null}
          {inst.bio ? <p>{inst.bio}</p> : null}
        </div>
      </div>
    </div>
  );
...
export function Horarios() {
  ...
  return (
    <section id="horarios" data-screen-label="Horarios">
      <div className="wrap">
        <Reveal>
          <div className="sec-kicker">Entrena con nosotros</div>
          <h2 className="sec-title">
            Nuestros <em>horarios</em>
          </h2>
        </Reveal>
...
export function Planes() {
  ...
  return (
    <section id="planes" data-screen-label="Planes">
      <div className="wrap">
        <Reveal>
          <div className="sec-kicker">Inscríbete</div>
          <h2 className="sec-title">
            Nuestros <em>planes</em>
          </h2>
        </Reveal>
        <div className="horario-grid">
          {planes.map((p, i) => (
